...
これらの環境に加え、Heroku のアカウントを作成済みであることを前提に説明を進めます。
...
次にプロジェクト名を決定します. 今回作成するプロジェクトは Springbot という名前にします.
...
![create_packege](https://user-images.githubusercontent.com/49631708/171631225-d486e4a6-0dfd-403e-8dab-800456ca952e.jpg)
...
プロジェクト内で上のコマンドを打つことに重要な意味があります。このコマンドを打つことで git の リモートリポジトリが自動で設定されます。以下のコマンドで確認してみてください。
...
今の状態で push すると起動が失敗したというエラーが出ると思います。

この理由は設定ファイルが不足しているからです。STS へ戻り、このプロジェクト内に 2 つの設定ファイルを追加すればこのエラーがなくなり、正しくデプロイできるようになります。
...
アプリの細かい動きの説明は割愛しますが、簡単に説明すると、Users テーブルから登録されているユーザ情報を取得して、表形式で表示するアプリになります。
...
ではこれから Heroku 上での PostgreSQL の設定に入りますが、このアプリを一から作るのではなく、リポジトリをクローンしてきた人用に説明を加えますので、一旦ここまできた人は[ここ](#add-on-heroku-postgres)から読み進めてください。

# Instructions for people who clone this repository
...
クローンできたら STS を開き、このプロジェクトを import します。
...
ブラウザが起動し、ログイン画面が表示されるのでログインします。色々認証がありますが、頑張ってログインしてください。ログインが完了したら、Heroku 上でアプリを作成します。アプリ名は使用されていない名前を用いる必要があるので、springbot-[任意の文字列]で、他と被らないようにしておきましょう。また大文字は使えないので小文字で設定しましょう。
...
このデータベースの URL はプロジェクト内の application.yml 内で指定するのですが、環境によってこの URL が変化する場合があるため、コピーすることは推奨しないと公式に書かれています。ゆえにデータベースの各種情報 (URL, userName, password) は環境変数として作成し、それを参照するように記述します。以下のコマンドを実行しましょう。

```
~ Springbot $ heroku run echo \$JDBC_DATABASE_URL
```

JDBC＿DATABASE_URL の部分はこの文字列のまま実行してください。先ほど確認した URL に置換する訳ではありません。
...
先ほど実行した echo コマンドのおかげで JDBC_DATABASE_URL から各種情報を参照できるようになっています。
...
上の表示に切り替わったら接続完了です。使い方はローカルの PostgreSQL と全く同じです。\d でテーブルの確認、\q で切断もできます。以下の SQL を実行してデータを作成しましょう。
...
以下の画面が表示されたら完了です。あとは好きなようにアプリを改良して遊びましょう！

![app-complate](https://user-images.githubusercontent.com/49631708/171633888-53e3c11e-a4dd-4f4f-bcb1-bd8468c9f502.png)
